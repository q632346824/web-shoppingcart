<template>
  <div >
    ...
    <div class="container">
              ...
              <li v-for="item in cartList" :key="item._id">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <div href="javascipt:;" class="checkbox-btn item-check-btn" v-bind:class="{'check':item.checked=='true'}" @click="editCart('checked',item)">
                  ...
                  <div class="cart-item-pic">
                    <img v-bind:src="getImgUrl(item.productImage)" v-bind:alt="item.productName">
                  </div>
                ...
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a href="javascript:;" class="item-edit-btn">
                      <div @click="delCartConfirm(item.productId)">Delete</div>
                    </a>
                  </div>
                </div>
              </li>
      ...
      <p slot="message">Are you sure to delete this data?</p>
      <div slot="btnGroup">
        <a class="btn btn--m" href="javascript:;" @click="delCart">Yes</a>
        <a class="btn btn--m btn--red" href="javascript:;" @click="modalConfirm = false">No</a>
      </div>
    </modal>
...
export default {
  ...
  methods:{
    init(){
      axios.get("/cartList").then((response)=>{
    ...
    delCart(){
      axios.post('/deleteItem',{productId:this.productId}).then(response=>{
        let res=response.data
        if(res.status==0){
          this.modalConfirm=false
          this.init()
    ...
    toggleCheckAll(){
      ...
      this.cartList.forEach((item)=>{
      item.checked=(flag=='true')?'true':'false'
      })

      axios.post("/editCheckAll",{
        checkAll:(flag=='true')?'true':'false'
        }).then(response=>{
