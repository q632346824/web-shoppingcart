<template>
  ...
  <nav-bread>
    <span slot="bread">confirm Order</span>
  </nav-bread>
      ...
      <div class="item-list-wrap confirm-item-list-wrap">
        <div class="cart-item order-item">
          ...
          <ul class="cart-item-list">
            <li v-for="(item,index) in filteredCartList" :key=index>
              <div class="cart-tab-1">
                <div class="cart-item-pic">
                  <img :src="getImgUrl(item.productImage)" alt="">
                </div>
...
export default {
  ...
  computed:{
    ...
    total(){
      return this.shipping+this.subtotal-this.discount+this.tax
    }
  },
  created(){
    this.init()
  ...
  methods:{
    ...
    payment(){
      ...
      axios.post('/payment',{ addressId:addressId, orderTotal:this.total}).then(response=>{
        let res=response.data
        if(res.status=='0'){
