      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li v-for="(item,index) in addressListFilter" :key="item.addressId" v-bind:class="{'check':checkIndex==index}" @click="checkIndex=index;selectedAddress=item.addressId">
                  <dl>
                    <dt>{{item.userName}}</dt>
            ...
            <div class="shipping-addr-more">
              <div class="addr-more-btn up-down-btn" @click="expand" v-bind:class="{'open':limit>3}">
                more
                <i class="i-up-down">
          ...
          <div class="next-btn-wrap" >
            <router-link class="btn btn--m btn--red" :to="{path:'orderConfirm',query:{'addressId':selectedAddress}}">Next</router-link>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods:{
    init(){
      axios.get("/addressList").then(response=>{
    ...
    delAddress(){
      axios.post("/delAddress",{ addressId:this.addressId }).then(response=>{
        let res=response.data
        if(res.status=='0'){
          this.isMdShow=false
          this.init()
